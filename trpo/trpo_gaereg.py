...
def main(env_name, seed=1, run_name=None):
    ...
    logger = LoggerData('trpo_gaereg_' + str(alpha) + '_' + str(kappa), env_name, run_name)
    print()
    ...
    for itr in range(maxiter):
        ...
        pi_loss_before = session.run(loss_pi, {obs: paths["obs"], pi.act: paths["act"], old_log_probs: old_lp, advantage: a_values - alpha*kappa**itr*reg_values})
        solver.step({obs: paths["obs"], pi.act: paths["act"], advantage: a_values - alpha*kappa**itr*reg_values, old_log_probs: old_lp, pi.old_mean: old_mean, pi.old_std: old_std})
        pi_loss_after = session.run(loss_pi, {obs: paths["obs"], pi.act: paths["act"], old_log_probs: old_lp, advantage: a_values - alpha*kappa**itr*reg_values})
