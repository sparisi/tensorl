'''
Same as sac2.py + entropy projection as in https://www.ias.informatik.tu-darmstadt.de/uploads/Team/RiadAkrour/icml19_sub.pdf
'''
...
def main(env_name, seed=1, run_name=None):
    ...
    loss_v = tf.losses.mean_squared_error(v.output[0], q.output[1])
    ...
    optimize_pi = tf.train.AdamOptimizer(lrate_pi).minimize(loss_pi, var_list=mean.vars+[l])
    ...
    logger = LoggerData('sac2_hproj', env_name, run_name)
    print()
